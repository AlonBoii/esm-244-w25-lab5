...
minimum values in columns are -999.000 which are probably NAs

variety should be a factor or real names
...
```{r}
scatter <- ggplot(seeds_df, aes(x = a, y = asym, color = variety, shape = variety)) +
  geom_point()
...
```{r}
scatter1 <- ggplot(seeds_df, aes(x = l_k, y = w_k, color = variety, shape = variety)) +
  geom_point()
...
```{r}
#| fig-cap: Based on this knee graph we would select 3 groups to initiate a k-means cluster because the kink in the graph is most visible at 3 clusters, after 3 groups there isn't much change
...
2. drop NAs (note: we don't have any in this case)

3. select only the numeric 

4. scale the data

5. get the distance of the scale data 
